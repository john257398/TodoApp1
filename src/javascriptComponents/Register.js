...
class Register extends Component{
 ...
 if(this.state.userValue=='' ||this.state.email=='' || this.state.passwordValue=='' || this.state.confirmPassword==''){
         ...
         <div className='register-container'>
          <form>
             ...
             <div className='inputBody3'><div className='regIcon3'> <FaAt /></div><input autoFocus type='text' placeholder='Email Address' value={this.state.email} onChange={this.changeEmail} className='email'/></div>
              ...
              <div className='buttonParent2'><button className='signUp' onClick={this.AddTodo}>REGISTER</button></div></div> 
             <div className='accountParent'> <p className='haveAccount'>Have an account? <span className='sign' onClick={this.signAccount}>Sign in</span></p></div>
          </form>
         </div>
        </>
        )
