...
class Inbox extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      removeTodo: false,
      putTodo: "",
      showingDate: new Date().toLocaleString("en-us", {
        month: "long",
        day: "numeric",
        year: "numeric",
      }),
  ...
  async editIt(keyer) {
    ...
    for (var x of putTodo) {
      if (x[3] === keyer) {
        var newLocal = [[x[0], x[1], x[2], x[6], this.state.getIndex]];
      }
     else if (newLocal[0][1] === x[1]) {
          ...
          <div className="map" id={value[5]} key={index}>
            <div className="checkAlign">
              <div
                ...
                keyer={value[3]}
                key={index}
                onClick={() => this.checkedIt(value[3])}
              ></div>
              <FaCheck
                className="checked"
                id={value[7]}
            ...
            <div className="headDiv">
              <div className="headAlign">
                <p className="theHead">{value[0]}</p>
              </div>
              <div className="bodyAlign">
                <p className="theBody">{value[1]}</p>
              </div>
            </div>
            <div className="iconAlign">
              ...
              <VscPreview
                ...
                key={index}
                keyer={value[3]}
                onClick={() => this.previewIt(value[3])}
              >
                preview
              </VscPreview>
            </div>
            <div className="timeAlign">
              <p className="timeMap">{value[2]}</p>
            </div>
            <editTodo
              keyTopic={value[0]}
              keyInfo={value[1]}
        ...
        <AnimatedRoutes>
          <div className="all">
            ...
            <div className="coversAll">
              <div className="theInbox">
                <h2>INBOX</h2>
              </div>
              <div
                className="noTodo"
                style={{ display: this.state.noTodoAdded }}
              >
                No Todo Added Yet
              </div>
              <div className="todoContent">{listItems}</div>
              <button className="addNew" onClick={this.showAddTodo}>
